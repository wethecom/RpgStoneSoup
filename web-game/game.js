...
function placeCastleFootprint(tiles, blockers) {
  ...
  for (let tries = 0; tries < 80; tries++) {
    ...
    // 4. main gate -- a T.GATE in the middle of the south curtain
    const gateX = x + (w >> 1), gateY = y + h - 1;
    tiles[gateY][gateX] = T.GATE;
    // approach south of the gate
    for (let ay = 1; ay <= 3; ay++) {
      const yy = gateY + ay;
      if (yy >= 1 && yy < MAP_H - 1) tiles[yy][gateX] = T.FLOOR;
    }
...
function ensureSurfaceChunk(cx, cy) {
  ...
  const lvl = newLevel("Surface", 1, 2, { cx, cy });
  ...
  for (const bm of (lvl.buildingMons || [])) {
    ...
    if (def) monsters.push(makeMonster(def, bm.x, bm.y));
  }
...
function enterLevel(branch, depth, arriveAt, coord) {
  ...
  if (cached) {
    ...
    for (const bm of (lvl.buildingMons || [])) {
      const def = monsterDefByName(bm.defName);
      if (def) G.monsters.push(makeMonster(def, bm.x, bm.y));
    }
...
function tryMovePlayer(dx, dy) {
  ...
  // a locked wooden door must be bashed
  ...
  if (here === T.WELL) {
    ...
    if (isDrained) { logMsg("The fruit's all gone.", "dim"); cancelWalk(); }
      ...
      if (chance(0.15)) {
        ...
        logMsg("You eat the fruit (+" + heal +
               " HP) but one was spoiled -- poisoned!", "warn");
      } else {
        logMsg("You eat the cached fruit (+" + heal + " HP).", "good");
      }
      sfx("quaff");
      drainPOI(); cancelWalk();
    }
...
function gameOver(won, killer) {
  ...
  document.getElementById("over-body").innerHTML =
    (won
      ? "You reached the Orb of Zot at the bottom of the Dungeon and " +
        "escaped with it. A genuine win.<br><br>"
      : "Slain by a " + killer + " in the " +
        levelLabel(G.branch, G.depth) + ".<br><br>") +
    `<span class="stat">${p.name}</span><br>` +
    `Experience level ${p.xl} &nbsp;&middot;&nbsp; ` +
    `${p.kills} monsters slain<br>` +
    `${p.gold} gold &nbsp;&middot;&nbsp; ${G.turn} turns<br>` +
    `Last seen in the ${levelLabel(G.branch, G.depth)}`;
  showScreen("over");
}
...
function tileGlyph(t) {
  switch (t) {
    ...
    case T.WATER: return { ch: "~", col: "#3a6ea5" };
...
function render() {
  ...
  if (ctx) {
    ...
    const cellFloorSet = (wx, wy) => {
      if (!buildingFloors) return floorSet;
      if (isSurface) {
        const ccx = Math.floor(wx / MAP_W), ccy = Math.floor(wy / MAP_H);
        const lx = wx - ccx * MAP_W, ly = wy - ccy * MAP_H;
        const dd = chunkData(ccx, ccy);
        const cb = buildingOuterAt(dd.level, lx, ly);
        if (!cb) return floorSet;
    ...
    for (let vy = 0; vy < VIEW_H; vy++) {
      for (let vx = 0; vx < VIEW_W; vx++) {
        ...
        const artName = (!isSurface && lvl.tileArt) &&
                        lvl.tileArt[y * MAP_W + x];
        const artRel = artName && vt && vt[artName];
        ...
        if (t === T.STAIRS_DOWN || t === T.STAIRS_UP) {
          ...
          rel = (t === T.STAIRS_DOWN) ? dn.stairs_down : dn.stairs_up;
        } else if (t === T.BRANCH) {
          drawCell(ctx, floorBase, null, null, sx, sy);
          rel = dn.stairs_down;        // a branch passage leads downward
        } else if (t === T.DOOR || t === T.DOOR_OPEN ||
                   t === T.DOOR_LOCKED || t === T.DOOR_STEEL ||
                   t === T.GATE) {
          drawCell(ctx, floorBase, null, null, sx, sy);
          // every closed door uses the closed-door sprite; the glyph
          // colour is what tells locked / steel / gate apart in ASCII
          rel = (t === T.DOOR_OPEN) ? dn.door_open : dn.door_closed;
        } else if (t === T.TREE) {
          drawCell(ctx, floorBase, null, null, sx, sy);
    ...
    if (hoverOnScreen && seenAt(hoverTile.x, hoverTile.y)) {
      ...
      if (desc) {
        ...
        ctx.textBaseline = "middle";
        const pad = 6, bh = 21;
        const bw = ctx.measureText(desc).width + pad * 2;
        let bx = hx + TILE + 4, by = hy + (TILE - bh) / 2;
        ...
        ctx.fillText(desc, bx + pad, by + bh / 2 + 1);
...
function renderShop() {
  ...
  if (!stock.length) {
    ...
    stock.forEach((it, i) => {
      ...
      html += `<div class="inv-row"><span class="inv-slot">` +
        String.fromCharCode(97 + i) + `)</span>` +
        `<span class="inv-val${afford ? "" : " inv-empty"}">` +
        `<b>${displayName(it)}</b> &mdash; ${it.price} gold</span></div>`;
    });
  }
  ...
  if (sellable.length) {
    html += `<hr><div class="inv-row"><span class="inv-slot">Sell` +
      `</span><span class="inv-val inv-empty">uppercase letter ` +
      `sells from your pack</span></div>`;
    sellable.forEach(({ it, i, price }) => {
      ...
      html += `<div class="inv-row"><span class="inv-slot">${letter})` +
        `</span><span class="inv-val">${desc}${qty} &mdash; ` +
        `${price} gold</span></div>`;
    });
  }
  html += `<div class="inv-here" style="margin-top:8px">` +
    `Lowercase letter buys; uppercase sells.</div>`;
  el.innerHTML = html;
}
...
function describeTileAt(x, y) {
  ...
  if (vis) {
    ...
    if (m) {
      return m.name + " — " + Math.max(0, m.hp) + "/" + m.hpMax +
             " HP" + (m.feared > 0 ? ", fleeing" :
                      (m.awake ? "" : ", asleep"));
    }
...
function renderInventory() {
  ...
  if (p.pack.length) {
    html += `<div class="inv-row"><span class="inv-slot">Backpack</span>` +
      `<span class="inv-val inv-empty">a letter wields / wears / ` +
      `quaffs / reads</span></div>`;
    p.pack.forEach((it, i) => {
...
function ensureQuestForNPC(npc) {
  if (!npc || npc.kind !== "questgiver") return null;
  if (npc.questId) {
